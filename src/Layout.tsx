  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 flex flex-col">
        {/* Header */}
        <header className={`sticky top-0 z-10 h-16 bg-white border-b border-gray-200 grid grid-cols-[auto_1fr_auto] items-center px-4 gap-2 transition-transform duration-200 ${headerHidden ? '-translate-y-full' : 'translate-y-0'}`}>
          {/* Left: menu buttons + logo */}
          <div className="flex items-center gap-2 min-w-0">
            ...
            <Link to="/" className="inline-flex items-center gap-2">
              <img src="/logo.png" alt="Data Metallogenica" className="h-7 sm:h-9 w-auto" />
            </Link>
          </div>

          {/* Center: title */}
          <div className="flex justify-center overflow-hidden px-2 min-w-0">
            <span className="font-bold text-base sm:text-2xl leading-tight truncate whitespace-nowrap max-w-[70vw] sm:max-w-none" style={{ color: '#3b403a' }}>Global Ore Deposits</span>
          </div>

          {/* Right: powered by (hidden on mobile, keeps grid for centering) */}
          <div className="hidden sm:flex items-center justify-end gap-2 text-xs text-gray-600">
